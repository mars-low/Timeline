...
class Timeline:
    ...
    def create_eras(self, y_era, y_axis, height):
        ...
        for era in eras_data:
            ...
            line0 = self.drawing.add(
                self.drawing.line((x0, 0), (x0, y_axis), stroke=fill, stroke_width=0.5)
            )
            line0.dasharray([5, 5])
            line1 = self.drawing.add(
                self.drawing.line((x1, 0), (x1, y_axis), stroke=fill, stroke_width=0.5)
            )
            line1.dasharray([5, 5])
            # create horizontal arrows and text
            horz = self.drawing.add(
                self.drawing.line(
                    (x0, y_era), (x1, y_era), stroke=fill, stroke_width=0.75
                )
            )
    ...
    def get_markers(self, color):
        ...
        if color in self.markers:
            ...
            start_marker = self.drawing.marker(
                insert=(0, 3), size=(10, 10), orient="auto"
            )
            start_marker.add(self.drawing.path("M6,0 L6,7 L0,3 L6,0", fill=color))
            self.drawing.defs.add(start_marker)
            end_marker = self.drawing.marker(
                insert=(6, 3), size=(10, 10), orient="auto"
            )
    ...
    def create_main_axis(self):
        ...
        self.g_axis.add(
            self.drawing.line(
                (0, 0), (self.width, 0), stroke=Colors.black, stroke_width=3
            )
        )
        # add tickmarks
    ...
    def add_axis_label(self, dt, label, **kwargs):
        ...
        if add_tick:
            ...
            self.g_axis.add(
                self.drawing.line((x, -dy), (x, dy), stroke=stroke, stroke_width=2)
            )
        # add label
    ...
    def create_callouts(self):
        ...
        for event_date in sorted_dates:
            ...
            i = len(prev_x) - 1
            left = x - (
                self.get_text_metrics("Helevetica", 6, event)[0]
                + self.callout_size[0]
                + self.text_fudge[0]
            )
            ...
            self.g_axis.add(
                self.drawing.path(
                    path_data, stroke=event_color, stroke_width=1, fill="none"
                )
            )
            self.g_axis.add(
                self.drawing.text(
                    event,
                    insert=(
                        x - self.callout_size[0] - self.text_fudge[0],
                        y + self.text_fudge[1],
                    ),
                    stroke="none",
                    fill=event_color,
            ...
            self.add_axis_label(
                event_date, str(event_date[0]), tick=False, fill=Colors.black
            )
            self.g_axis.add(
                self.drawing.circle(
                    (x, 0), r=4, stroke=event_color, stroke_width=1, fill="white"
                )
            )
            prev_x.append(x)
            prev_level.append(k)
